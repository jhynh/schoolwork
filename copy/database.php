...
class TextRowType implements AbstractRowType {
  ...
  public function getCreateSQL() {
    return "ALTER TABLE `student_graderz` ADD `$this->name` VARCHAR(255) NOT NULL DEFAULT '' COMMENT 'TextRowType';";
  }
}
...
class EnumRowType implements AbstractRowType {
  ...
  public function getCreateSQL() {
    ...
    return "ALTER TABLE `student_graderz` ADD `$this->name` enum('A', 'B', 'C', 'D', 'F', 'U') COMMENT 'GradeRowType' NOT NULL DEFAULT 'U'";
  }
  public function formatData($d) {
    return $d;
  }
...
//by extending, we;re able to access the methods
//by seperating, we can...huh.. why seperate it?
...
class PhoneRowType extends TextRowType {
  ...
  public function getCreateSQL() {
    return "ALTER TABLE `student_graderz` ADD `$this->name` varchar(12) COMMENT 'PhoneRowType' NOT NULL DEFAULT '000-000-0000'";
  }
}
...
class FileRowType implements AbstractRowType {
  ...
  public function getCreateSQL() {
    ...
    return "ALTER TABLE student_graderz ADD $this->name MEDIUMBLOB COMMENT '$class'";
  }
  //Create a File in Memory for Users to Download on Client Side with JavaScript
  public function displayData($d) {
...
class GradesDatabase {
  ...
  function __construct() {
    $this->dbh = new PDO('mysql:dbname=members;host=localhost', 'jason', 'jh9868');
    $this->dbh->prepare("CREATE TABLE IF NOT EXISTS `student_graderz` ( `name` varchar(255) NOT NULL COMMENT 'TextRowType', PRIMARY KEY (`name`) ) DEFAULT CHARSET=utf8mb4")->execute();
  }

  public function selectAll($sortBy = NULL, string $order = GradesDatabase::Descending):PDOStatement {
    $stmt = "SELECT * FROM `student_graderz`";
    if (isset($sortBy))
  ...
  public function getRowTypeByName(string $s):AbstractRowType {
    $stmt = "SELECT CHARACTER_MAXIMUM_LENGTH,COLUMN_COMMENT FROM `information_schema`.`columns` WHERE `TABLE_NAME` = 'student_graderz' AND `COLUMN_NAME` = :column_name;";
    $sth = $this->dbh->prepare($stmt);
    $sth->bindParam(":column_name", $s);
    if (!$sth->execute())
      throw new RuntimeException("Unknown Column");
    $vals = $sth->fetch();    //fetch the row
    $maxlen = $vals[0];
    $type = $vals[1];
    try {
      //echo "<a>".$type."</a>"; //--------------------------------------------------------------
      return new $type($s);   //calls the correct type
    } catch (Error $e) {
  ...
  public function createRow(array $o) {
    ...
    $stmt = "INSERT INTO student_graderz (";
    $stmt .= implode(',', $keys);
    $stmt .= ") VALUES (";
    foreach ($keys as $key)
      $stmt .= ":$key, ";
  ...
  public function updateRow(string $name, array $o) {
    ...
    $stmt = "UPDATE `student_graderz` SET ";
    foreach ($keys as $key)
      $stmt .= "$key = :$key, ";
    $stmt = substr($stmt, 0, -2);
  ...
  public function deleteRow($o) {
    $name = $o["name"];
    $stmt = "DELETE FROM `student_graderz` WHERE `name` = :name;";
    $sth = $this->dbh->prepare($stmt);
    $sth->bindParam(":name", $name);
    $res = $sth->execute();
    if (!$res)
 ...
 */
  ...
  public function getCols() {
    $stmt = "SELECT COLUMN_NAME as name,COLUMN_COMMENT as type FROM `information_schema`.`COLUMNS` WHERE `TABLE_NAME` = 'student_graderz';";
    $sth = $this->dbh->prepare($stmt);
    if (!$sth->execute())
  ...
  public function deleteCol($d) {
    ...
    if ($col == 'name')
      return;
    $stmt = "ALTER TABLE `student_graderz` DROP `$col`;";
    $sth = $this->dbh->prepare($stmt);
    if (!$sth->execute())
      throw new Exception(print_r($sth->errorInfo(), true), $sth->errorInfo()[0]);
  }
