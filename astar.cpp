...
#include<utility>
#include <memory>
class PathFinding_Astar : public olc::PixelGameEngine
{
public:
	PathFinding_Astar()
	{
...
protected:
	virtual bool OnUserCreate()
	{
		...
		//position the start and end markers so they are not nullptr, my heuristic
		...
		for (int x = 0; x < nMapWidth; x++)
			for (int y = 0; y < nMapHeight; y++)
			{
				...
				FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, 
					...
					nodes[y * nMapWidth + x].bObstacle ? olc::WHITE : olc::BLUE); //PIXEL_HALF

				if (nodes[y * nMapWidth + x].bVisited)
					FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, (x + 1)*nNodeSize - nNodeBorder, (y + 1)*nNodeSize - nNodeBorder, olc::BLUE);//PIXEL_SOLID

				if(&nodes[y * nMapWidth + x] == nodeStart)
					FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, (x + 1)*nNodeSize - nNodeBorder, (y + 1)*nNodeSize - nNodeBorder, olc::GREEN);//PIXEL_SOLID

				if(&nodes[y * nMapWidth + x] == nodeEnd)
					FillRect(x*nNodeSize + nNodeBorder, y*nNodeSize + nNodeBorder, (x + 1)*nNodeSize - nNodeBorder, (y + 1)*nNodeSize - nNodeBorder, olc::RED);//PIXEL_SOLID						
